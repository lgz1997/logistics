/**
 * Copyright (C), 2015-2020, XXX有限公司
 * FileName: FreightcalculationServiceImpl
 * Author:   没闹~
 * Date:     2020/10/7 14:38
 * Description:
 * History:
 * <author>          <time>          <version>          <desc>
 * 作者姓名           修改时间           版本号              描述
 */
package com.ytc.service;

import com.alibaba.dubbo.config.annotation.Service;
import com.ytc.mapper.FreightcalculationMapper;
import com.ytc.model.Freightcalculation;
import com.ytc.model.Suser;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;

/**
 * 〈一句话功能简述〉<br> 
 * 〈〉
 *
 * @author 没闹~
 * @create 2020/10/7
 * @since 1.0.0
 */
@Service
@Component
public class FreightcalculationServiceImpl implements FreightcalculationService{

    @Autowired
    private FreightcalculationMapper freightcalculationMapper;

    @Override
    public Freightcalculation sel(Suser suser) {
        return freightcalculationMapper.sel(suser);
    }

    @Override
    public void upf(Freightcalculation freightcalculation) {
        freightcalculationMapper.upf(freightcalculation);
    }

    @Override
    public void add(Integer id) {
        freightcalculationMapper.add(id);
    }

    @Override
    public Freightcalculation queryCalculation(Integer suserid) {
        return freightcalculationMapper.queryCalculation(suserid);
    }
}